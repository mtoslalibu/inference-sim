...
// router_adaptive_expanded.go — Drop-in replacement for sim/routing.go
//
// Adaptive-expanded (golden): 5-scorer regime-detection router using
// precise-prefix-cache, load-aware, active-requests, running-requests,
// and kv-utilization scorers. Same regime detection as adaptive-v2 but
// with richer load signals for finer-grained routing.
//
...
//
// ADAPTIVE-EXPANDED (GOLDEN) MODIFICATION: 5-scorer regime-detection router.
// Same regime detection as adaptive-v2 (cacheSpread, avgKVUtil) but with
// richer load signals: active-requests and running-requests join load-aware.
//
//   Regime 1 — Cache-affinity (cacheSpread > 0.1):
//     ppc:4/6, active-requests:1/6, load-aware:1/6
//     Cache locality dominant, minimal load balancing via synchronous signals.
//
//   Regime 2 — Memory-aware (avgKVUtil > 0.7):
//     active-requests:2/5, load-aware:1/5, running-requests:1/5, kv-utilization:1/5
//     Under pressure, disable prefix affinity; use all load signals + memory.
//
//   Regime 3 — Load-balance (default):
//     active-requests:2/4, load-aware:1/4, running-requests:1/4
//     Balanced load distribution with three complementary load signals.
//
// Scorers are instantiated from the policy YAML. The YAML weights are ignored
// at runtime; regime detection overrides them.
...
// ADAPTIVE-EXPANDED: Per-request regime detection with 5 scorers.
func (ws *WeightedScoring) Route(req *Request, state *RouterState) RoutingDecision {
	...
	// ###########################################################################
	// ### EVOLVED ALGO LOGIC — adaptive-expanded / golden (5-scorer regime detection)
	// ###
	...
	case cacheSpread > 0.1:
		// Regime 1: Cache-affinity — strong prefix signal.
		// ppc:4, ar:1, la:1, rr:0, kvu:0
		regimeWeights = []float64{4.0, 1.0, 1.0, 0.0, 0.0}
		regime = "cache-affinity"
	case avgKVUtil > 0.7:
		// Regime 2: Memory-aware — disable prefix, all load + memory signals.
		// ppc:0, la:1, ar:2, rr:1, kvu:1
		regimeWeights = []float64{0.0, 1.0, 2.0, 1.0, 1.0}
		regime = "memory-aware"
	default:
		// Regime 3: Load-balance — balanced load distribution.
		// ppc:0, la:1, ar:2, rr:1, kvu:0
	...
	// ###########################################################################
	// ### END EVOLVED ALGO LOGIC — adaptive-expanded / golden
	// ###########################################################################

	return NewRoutingDecisionWithScores(
		snapshots[bestIdx].ID,
		fmt.Sprintf("adaptive-expanded/%s (score=%.3f)", regime, bestScore),
		scores,
	)
}
