#!/usr/bin/env bash
# sim2real_golden — Portable Router Comparison Benchmark
#
...
#   bash compare.sh           # Run core 3 workloads (recommended)
...
# Core workloads: 3 production-representative scenarios with clean reproducibility.
# These show clear gains, no timeouts, and cover distinct failure modes.
CORE_WORKLOADS="
workload_fm2a_groups_gt_instances
workload_fm3_burst
workload_fm6_cold_pressure
"

# Extended workloads: remaining 4 for comprehensive coverage.
EXTENDED_WORKLOADS="
workload_fm1_prefix_pileon
workload_fm2b_groups_lt_instances
workload_fm4_multiregime
...
if [ "$RUN_ALL" = "true" ]; then
    ...
    wl_mode="3 core workloads (use --all for all 7)"
fi
...
echo "============================================================"
echo " sim2real_golden — Portable Router Comparison"
echo "============================================================"
echo ""
echo "Model:       $MODEL"
...
for router_entry in $ROUTERS; do
    ...
    if [ "$router_file" = "STOCK" ]; then
        # Restore stock routing.go (in case previous router was swapped)
        cp "$BACKUP_ROUTING" "$ORIGINAL_ROUTING"
...
# --- Restore routing.go (trap also does this, but be explicit) ---
cp "$BACKUP_ROUTING" "$ORIGINAL_ROUTING"
