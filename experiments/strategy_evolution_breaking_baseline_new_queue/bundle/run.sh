#!/usr/bin/env bash
# Adaptive-v2/v3 vs Adaptive-v1 vs Baseline 2:1:1: Load-Aware Benchmark
#
# Runs 4 policies across 7 workloads, 3 seeds each.
# Total: 4 x 7 x 3 = 84 runs.
#
# Usage:
#   cd experiments/strategy_evolution_breaking_baseline_new_queue/bundle
#   bash run.sh
#
# Or from repo root:
#   bash experiments/strategy_evolution_breaking_baseline_new_queue/bundle/run.sh
...
adaptive-v1:policy_adaptive_v1.yaml
adaptive-v2:policy_adaptive_v2.yaml
adaptive-v3:policy_adaptive_v3.yaml
"
...
echo "============================================================"
echo " Load-Aware Adaptive Router Benchmark"
echo "============================================================"
echo ""
echo "Model:       $MODEL"
...
for wl_entry in $WORKLOADS; do
    ...
    for pol_entry in $POLICIES; do
        ...
        for seed in $SEEDS; do
            ...
            cd "$REPO_ROOT"
            # 90s timeout per run — kills preemption near-livelock (issue #963)
            ./blis run \
                --model "$MODEL" \
                --hardware "$HARDWARE" \
                --tp "$TP" \
            ...
            blis_pid=$!
            # Watchdog: kill after 90s
            ( sleep 90 && kill $blis_pid 2>/dev/null ) &
            watchdog_pid=$!
            if wait $blis_pid 2>/dev/null; then
                ...
                if [ "$exit_code" = "143" ]; then
                    echo "TIMEOUT (preemption livelock, issue #963)"
                else
