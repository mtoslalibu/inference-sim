...
- **INV-1 Request conservation**: `injected_requests == completed_requests + still_queued + still_running + dropped_unservable + timed_out + routing_rejections + gateway_queue_depth + gateway_queue_shed + gateway_queue_rejected` at simulation end. Full pipeline: `num_requests == injected_requests + rejected_requests`.
...
## Recent Changes
- Per-priority-band queues + FlowControlAdmission policy (#1191): `GatewayQueue` replaced with per-band, per-flow queue structure keyed by `FlowKey{TenantID, Priority}`. `FlowControlAdmission` policy merges admission and queuing when `--flow-control` is enabled — admission IS the queue, matching llm-d/GIE architectural parity. Per-band capacity enforcement via `--per-band-capacity` flag (0=unlimited). Dispatch follows `--dispatch-order`: `priority` iterates bands highest-first, `fifo` (default) picks globally-earliest arrival; within-band uses global-strict fairness (earliest seqID). `TenantBudgetAdmission` wraps `FlowControlAdmission` (budget check before enqueue). Closes #1191, part of #899.
...
- SLO class → vLLM priority injection in observe (#1208): When `slo_class` is set in the workload spec, `blis observe` now injects both the `x-gateway-inference-objective` header (for llm-d) and a `priority` body field (for vLLM) in every HTTP request. vLLM uses lower integers for higher urgency (min-heap), opposite of BLIS/llm-d. `SLOPriorityMap.InvertForVLLM(class)` computes the inverted priority via `maxPriority - Priority(class)`. Default mapping: critical→0, standard→1, batch→5, sheddable→6, background→7. Custom SLO priorities supported via `WithSLOPriorityMap()` option when constructing `RealClient`. Servers ignore what they don't use: vLLM ignores the header, llm-d (using vLLM FCFS backend) silently ignores the body priority field.
